...
- [Anomaly Analysis Chat (XiaoYi)](#anomaly-analysis-chat-xiaoyi)
...
### Anomaly Analysis Chat (XiaoYi)

A built-in two-stage anomaly-judgment LLM for interactive analysis of selected rounds. It uses "Skill Analysis" orchestration: streaming + tool calls so that forensic tools execute and render in real time as the model output is parsed, with the final verdict streamed token-by-token, greatly reducing perceived latency. See [Anomaly Analysis Chat (XiaoYi)](#anomaly-analysis-chat-xiaoyi).
...
| `/anomaly-chat` | Anomaly Analysis Chat | Select a round and let XiaoYi investigate step by step via tool calls | Entry passphrase |
...
"XiaoYi" is ClawAVC's built-in two-stage anomaly-judgment LLM for automatic log assessment and interactive anomaly analysis. On the Anomaly Analysis Chat page, after selecting a round, XiaoYi reads that round's full multi-dimensional behavior-trajectory audit data and investigates step by step via tool calls, outputting a structured compliance verdict.

### How it works

XiaoYi uses "Skill Analysis" orchestration, whose core trait is **streaming + real-time tool rendering**:

```
User selects a round
    ↓
Load that round's full multi-dimensional audit data (system + data)
    ↓
Streaming LLM request (stream + tools)
    ↓   parse tool_calls as model output streams in
    ├─ a tool name is parsed → execute that tool immediately → result pushed to UI in real time
    ├─ text content is parsed → streamed token-by-token to the UI (final analysis)
    └─ stream ends           → assemble full tool_calls, append to message history for the next round
    ↓
(repeat for multi-round investigation) until the model stops calling tools and emits the final verdict
```

> Forensic tools only need the externally known `round_id` as input and do not depend on arguments generated by the model, so tool results can be **rendered one by one in real time** as they are parsed, without waiting for the whole model response to finish — greatly reducing perceived latency.

### XiaoYi Configuration

XiaoYi configuration keys (prefix `monitor_conf.`) are edited visually in the "Settings" dialog at the top-right of the Anomaly Analysis Chat page, or written directly via `PUT /api/monitor/config`. Available keys:

| Config key | Description | Default |
|------------|-------------|---------|
| `anomaly_llm_url_v2` | OpenAI-compatible endpoint (shared by chat and skill analysis); no LLM call if unset | none |
| `anomaly_llm_max_tokens` | Max tokens generated per LLM call | `2048` |
| `anomaly_llm_base_prompt` | Analysis instruction (how XiaoYi should analyze the data); default if empty | none |
| `anomaly_llm_system_prompt` | System persona (applies to all chats); default if empty | none |
| `anomaly_llm_skill_prompt` | **Skill-analysis-only** system prompt defining tool-use strategy, analysis steps and final output format; applies only to "Skill Analysis"; default if empty | none |
| `anomaly_llm_tool_compress` | Behavior when a tool result is too long: `true` = compress via LLM; `false` = truncate directly | `false` |
| `anomaly_llm_tool_compress_max` | Length threshold (chars) that triggers compression / truncation | `2000` |

> ⚠️ All keys above are in the backend `MONITOR_CONF_KEYS` whitelist and strictly validated when saved via the config API. XiaoYi has no separate model field and reuses the default model of the pointed-to service.

### Tool Result Handling

When a forensic tool returns content longer than `anomaly_llm_tool_compress_max`:
- **Switch on** (`tool_compress = true`): call the LLM to summarize/compress the result before feeding it back to XiaoYi;
- **Switch off** (`tool_compress = false`): truncate directly to the threshold and mark it "（结果过长，已截断）" / "(result too long, truncated)".

### Settings Dialog

The "XiaoYi Settings" dialog (760px wide, scrolls when content overflows) provides visual editing for all the above keys, grouped into:
- **Model Access**: endpoint URL, max tokens;
- **Prompt Layering**: analysis instruction / system persona / skill-analysis prompt (three independent layers, each overriding the default on demand);
- **Tool Result Compression**: switch + threshold input.

Clearing the chat (top-right "Clear" button) resets to the default greeting so it is excluded from the context sent to the LLM, ensuring a new analysis carries no historical residue.
...
│   │   │   ├── AnomalyChatPage.vue # Anomaly Analysis Chat (XiaoYi)

...
const formattedContent = computed(() => {
  ...
  const format = getContentFormat(props.content)
  
  if (format === 'jsonl') {
    return jsonlToHtml(props.content)
  } else if (format === 'json') {
    return jsonToJsonHtml(props.content)
  } else {
    return `<pre class="text-content">${escapeHtml(props.content)}</pre>`
  }
})
